...
def define_workshops_products():
    db.define_table('workshops_products',
        ...
        Field('AddToCartText',
            ...
            comment=os_gui.get_info_icon(
                  title=T("Change the text for the 'Add to cart' button. Shows 'Add to cart' when left empty."),
                  btn_icon='info')),
        Field('Donation', 'boolean',
            label=T("Donation based"),
            default=False,
            comment=os_gui.get_info_icon(
                  title=T("Shows 'Donation based' instead of the price in the shop."),
                  btn_icon='info')),
        format='%(Name)s')
...
def define_payment_batches():
    ...
    db.define_table('payment_batches',
        ...
        Field('BatchTypeDescription',
            readable=False,
            writable=False,
            requires=IS_IN_SET([['teacher_payments', T('Teacher payments')],
                                ['invoices', T('Invoices')],
                                ['category', T('Category')]])),
        Field('Status',
...
def define_invoices_items():
    db.define_table('invoices_items',
        ...
        Field('Quantity', 'integer',
            requires=IS_INT_IN_RANGE(1, 100000,
                     error_message=T("Enter a number between 1 and 100000")),
            default=1,
            label=T("Quantity")),
        Field('Price', 'double',
            represent=represent_float_as_amount,
