    ...
    <div className="box box-solid"> 
        ...
        <div className="box-header with-border">
            <h3 className="box-title">
                {intl.formatMessage({ id:"app.pos.classes.revenue.total.title" })}
                <small>
                    { ' ' }
                    { (teacher_payment.data.Status === 'verified') ?
                        <span>
                            <i className="text-green fa fa-check"></i> { ' ' }
                            {intl.formatMessage({ id:"app.pos.classes.revenue.total.verified"})}
                        </span> :
                        <span>
                            <i className="text-red fa fa-ban"></i> { ' ' }
                            {intl.formatMessage({ id:"app.pos.classes.revenue.total.not_verified" })}
                        </span>}
                </small>
            </h3>
        </div>
            ...
            <table className="table">
                ...
                <tbody>
                    <tr>
                        ...
                        <td>{currency_symbol} { ' ' } {revenue.total.amount}</td>
                    </tr>
                    <tr>
                        ...
                        <td>{ (teacher_payment.error) ? 
                                ...
                                currency_symbol +  ' ' + teacher_payment.data.ClassRate }
                        </td>
                    </tr>
                </tbody>
                <tfoot>
                    <tr>
                        <th>{intl.formatMessage({ id:"app.pos.classes.revenue.total.studio_revenue" })}</th>
                        <th>{currency_symbol} { ' ' } { 
                            (revenue.total.amount - teacher_payment.data.ClassRate) }
                        </th> 
                    </tr>
                </tfoot>
            </table>
