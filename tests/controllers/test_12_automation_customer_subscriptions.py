...
def test_automation_customers_exp_memberships_check_invoices(client, web2py):
    """
    ...
    url = '/test_automation_customers_subscriptions/' + \
          'test_check_memberships_subscriptions' + \
          '?month=2&year=2014&description=Membership_Feb'
    client.get(url)
    assert client.status == 200

    row = web2py.db.customers_memberships(id= 1)
    query = (web2py.db.customers_memberships.Startdate == row.Enddate+ timedelta(days= 1))
    assert query
    rows = web2py.db(query).select()
    for row in rows:
        invoice_membership_row = web2py.db.invoices_customers_memberships(customers_memberships_id = row.id)
        # cm = CustomerMembership(row.id)
        inv_row = web2py.db.invoices(id = invoice_membership_row.invoices_id)

        #Check if the invoice is filled with data from Customers_memberships correctly
        assert((web2py.db.invoices.MembershipPeriodStart == row.Startdate) &\
               (web2py.db.invoices.MembershipPeriodEnd == row.Enddate) &\
               (web2py.db.invoices.Description == 'Premium membership'))
        inv_items_rows = web2py.db.invoices_items(invoices_id= inv_row.id)
        for inv__items_row in inv__items_rows:
            query= (web2py.db.school_memberships_price.school_memberships_id == row.school_memberships_id)
            price = web2py.db(query).select(web2py.db.school_memberships_price.Price).first()
            assert (web2py.db.invoices_items.Price == price)
