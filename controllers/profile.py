...
#     {{pay_now = A(T('Pay now'), _href=URL('mollie', 'invoice_pay', vars={'iID': row.invoices.id}),
#                   _class='btn btn-success') if row.invoices.Status == 'sent' else ''}}
#     {{
#     pass}}
#
#     {{table.append(TR(TD(A(repr_row.invoices.InvoiceID, _href=URL('invoice', vars={'iID': row.invoices.id}))),
...
#                       TD(DIV(os_gui.get_button('print', URL('invoices', 'pdf', vars={'iID': row.invoices.id}),
#                                                btn_size=''),
