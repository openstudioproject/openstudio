...
# def attendance_get_search_form(clsID, date, name):
#     '''
...
#                    DIV(DIV(SPAN(_class='glyphicon glyphicon-remove grey'),
#                            _class='btn btn-default'),
#                        _class='input-group-btn',
#                        _id='no_table_name_clear'),
#                    _class='input-group'),
#                form.custom.end,
#                _class='col-md-4 no-padding-left')
#
#     return form
...
# @auth.requires(auth.has_membership(group_id='Admins') or \
#                auth.has_permission('update', 'classes_attendance'))
# def reservation_mail_teacher():
#     clsID = request.vars['clsID']
#     date_formatted = request.vars['date']
#     date = datestr_to_python(DATE_FORMAT, date_formatted)
#
#     result = class_get_teachers(clsID, date)
#     teID = result['teacher_id']
#     au_row = db.auth_user[teID]
#
#     left = [ db.auth_user.on(db.classes_reservation.auth_customer_id == \
#                              db.auth_user.id) ]
#     query = (db.classes_reservation.classes_id==clsID) & \
#             (db.classes_reservation.Startdate <= date) & \
#             ((db.classes_reservation.Enddate >= date) |
#              (db.classes_reservation.Enddate == None))
#
#     orderby = ~db.classes_reservation.TrialClass|\
#               ~db.classes_reservation.SingleClass|\
#               ~db.classes_reservation.Startdate|\
...
#                             left=left,
#                             orderby=orderby)
#     table = TABLE(TR(TH(T("Customer")), TH()), _class='table table-hover table-condensed')
#     for row in rows.render():
#         customers_name = SPAN(row.auth_user.display_name)
#         table.append(TR(TD(customers_name),
#                         TD(row.classes_reservation.ResType)))
#
#     ## Start generation of message
#     head = '<head>'
#     head += '<link rel="stylesheet" type="text/css" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.5/css/bootstrap.min.css">'
#     head += '</head>'
#     message = "<html>\n"
#     message += head
#     message += "<body>"
#     message +=  T("Dear") + " " + au_row.first_name + ",<br><br>"
#     message += T("Below is the expected attendance list for your class") + \
#                 " <i>" + get_classname(clsID) + "</i> " + T("on") + " <i>" + \
#                 unicode(date) + "</i>.<br><br>"
#     message += unicode(table) + "</p><br><br>"
#     message += unicode(spaces) + '<br><br>'
...
#     message += "<br><br><font size=2>" + \
#                T("This message is autogenerated by OpenStudio") + \
#                " - <a href='http://www.openstudioproject.com'>www.openstudioproject.com</a></font>"
#     message += "</body></html>"
#     # now send the mail
#     subcheck = db.classes_subteachers(classes_id=clsID, ClassDate=date)
#     if subcheck == None:
#         email_teacher = au_row.email
#     else:
#         teacher = db.auth_user(subcheck.auth_teacher_id)
#         email_teacher = teacher.email
#     check = MAIL.send(to=[email_teacher],
#           subject=T("Attendance list for your class on") + " " + unicode(date),
...
#
#     redirect(URL('attendance', vars={'clsID':clsID, 'date':date_formatted}))
